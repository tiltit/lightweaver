/****************************************************************************
** Meta object code from reading C++ file 'motorcontrol.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.0.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "motorcontrol.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'motorcontrol.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.0.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_MotorControl_t {
    QByteArrayData data[7];
    char stringdata[71];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_MotorControl_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_MotorControl_t qt_meta_stringdata_MotorControl = {
    {
QT_MOC_LITERAL(0, 0, 12),
QT_MOC_LITERAL(1, 13, 10),
QT_MOC_LITERAL(2, 24, 0),
QT_MOC_LITERAL(3, 25, 7),
QT_MOC_LITERAL(4, 33, 20),
QT_MOC_LITERAL(5, 54, 9),
QT_MOC_LITERAL(6, 64, 5)
    },
    "MotorControl\0gotNewLine\0\0newLine\0"
    "commandQueueDequeued\0queueSize\0onTic\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_MotorControl[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x05,
       4,    1,   32,    2, 0x05,

 // slots: name, argc, parameters, tag, flags
       6,    0,   35,    2, 0x08,

 // signals: parameters
    QMetaType::Void, QMetaType::QString,    3,
    QMetaType::Void, QMetaType::Int,    5,

 // slots: parameters
    QMetaType::Void,

       0        // eod
};

void MotorControl::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        MotorControl *_t = static_cast<MotorControl *>(_o);
        switch (_id) {
        case 0: _t->gotNewLine((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 1: _t->commandQueueDequeued((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->onTic(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (MotorControl::*_t)(QString );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&MotorControl::gotNewLine)) {
                *result = 0;
            }
        }
        {
            typedef void (MotorControl::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&MotorControl::commandQueueDequeued)) {
                *result = 1;
            }
        }
    }
}

const QMetaObject MotorControl::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_MotorControl.data,
      qt_meta_data_MotorControl,  qt_static_metacall, 0, 0}
};


const QMetaObject *MotorControl::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *MotorControl::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_MotorControl.stringdata))
        return static_cast<void*>(const_cast< MotorControl*>(this));
    return QObject::qt_metacast(_clname);
}

int MotorControl::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void MotorControl::gotNewLine(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void MotorControl::commandQueueDequeued(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
